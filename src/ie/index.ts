...
const validators: Record<string, Validator> = {
  AC: (ie) => {
    ...
    let sum = base
      .split('')
      .reduce((acc, digit, i) => acc + parseInt(digit ?? '0', 10) * (weights1[i] ?? 0), 0);
    let dv1 = 11 - (sum % 11);
    if (dv1 >= 10) {
      dv1 = 0;
    }
    if (dv1 !== parseInt(ie[11] ?? '0', 10)) {
      return false;
    }
    ...
    sum = ie
      .substring(0, 12)
      .split('')
      .reduce((acc, digit, i) => acc + parseInt(digit ?? '0', 10) * (weights2[i] ?? 0), 0);
    let dv2 = 11 - (sum % 11);
    if (dv2 >= 10) {
      dv2 = 0;
    }
    return dv2 === parseInt(ie[12] ?? '0', 10);
  },
  ...
  AL: (ie) => {
    ...
    const sum = ie
      .substring(0, 8)
      .split('')
      .reduce((acc, digit, i) => acc + parseInt(digit ?? '0', 10) * (weights[i] ?? 0), 0);
    const remainder = (sum * 10) % 11;
    const dv = remainder === 10 ? 0 : remainder;
    return dv === parseInt(ie[8] ?? '0', 10);
  },
  ...
  AM: (ie) => {
    ...
    const sum = ie
      ...
      .reduce((acc, digit, i) => acc + parseInt(digit ?? '0', 10) * (weights[i] ?? 0), 0);
    if (sum < 11) {
      return 11 - sum === parseInt(ie[8] ?? '0', 10);
    }
    const remainder = sum % 11;
  ...
  BA: (ie) => {
    ...
    const module = '679'.includes(ie[ie.length === 9 ? 1 : 0] ?? '0') ? 11 : 10;
    ...
    if (dv1 !== parseInt(ie[ie.length - 2] ?? '0', 10)) {
      return false;
    }
    ...
    return dv2 === parseInt(ie[ie.length - 1] ?? '0', 10);
  },
  ...
  GO: (ie) => {
    ...
    if (!['10', '11', '15'].includes(prefix)) {
      return false;
    }
    const base = ie.substring(0, 8);
    if (base === '11094402') {
      return ['0', '1'].includes(ie[8] ?? '0');
    }
    const weights = [9, 8, 7, 6, 5, 4, 3, 2];
    const sum = base
      .split('')
    ...
    const remainder = sum % 11;
    if (remainder === 0) {
      return parseInt(ie[8] ?? '0', 10) === 0;
    }
    if (remainder === 1) {
      const num = parseInt(base, 10);
      return parseInt(ie[8] ?? '0', 10) === (num >= 10103105 && num <= 10119997 ? 1 : 0);
    }
    return 11 - remainder === parseInt(ie[8] ?? '0', 10);
  },
  ...
  MG: (ie) => {
    ...
    const sum1 = newBase.split('').reduce((acc, digit, i) => {
      const product = parseInt(digit ?? '0', 10) * (weights1[i] ?? 0);
  ...
  RJ: (ie) => {
    ...
    const remainder = sum % 11;
    const dv = remainder <= 1 ? 0 : 11 - remainder;
    return dv === parseInt(ie[7] ?? '0', 10);
  },

  RN: (ie) => {
    if (![9, 10].includes(ie.length) || !ie.startsWith('20')) {
      return false;
    }
    ...
    const remainder = (sum * 10) % 11;
    const dv = remainder === 10 ? 0 : remainder;
    return dv === parseInt(ie[ie.length - 1] ?? '0', 10);
  },
  ...
  SP: (ie) => {
    ...
    sum = ie
      ...
      .reduce((acc, digit, i) => acc + parseInt(digit ?? '0', 10) * (weights2[i] ?? 0), 0);
    const dv2 = (sum % 11) % 10;
    return dv2 === parseInt(ie[11] ?? '0', 10);
  },
  ...
  TO: (ie) => {
    ...
    if (!['01', '02', '03', '99'].includes(ie.substring(2, 4))) {
      return false;
    }
